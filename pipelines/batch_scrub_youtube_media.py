"""Pipeline: batch_scrub_youtube_media — scrub every eligible media file in a folder.
...
per-file work to pipelines.scrub_youtube_media via shared.batch.run_dir_batch
— no scrub logic is duplicated here.

Given an input directory:
  1. Discover every file directly inside it with a recognised media extension
     (non-recursive).
  2. Run pipelines.scrub_youtube_media on each file, writing output into
     output_dir.
  3. Collect a per-file result; one failure does not stop the batch.

Mirrors the core loop of wrapper_scripts/process_youtube_inbox.sh (find
...
Config (pipelines/batch_scrub_youtube_media.yaml):
    verbose     — print progress (default: true)
    extensions  — file extensions treated as candidate inputs
    pipeline    — options forwarded to pipelines.scrub_youtube_media

Usage:
    uv run -m pipelines.batch_scrub_youtube_media /inbox /out
    uv run -m pipelines.batch_scrub_youtube_media /inbox /out --force
"""
...
def run(
    ...
    *,
    ...
    """Scrub every eligible media file directly inside input_dir.
...
def _cli() -> None:
    ...
    parser = argparse.ArgumentParser(description="Scrub every eligible media file in a folder.")
