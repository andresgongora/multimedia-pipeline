"""Tests: pipelines/batch_scrub_youtube_podcast.py — delegation, aggregation, error isolation.

Mocks pipelines.scrub_youtube_podcast.run (no network, no real media).

Usage:
    uv run test/pipelines/batch_scrub_youtube_podcast.py
"""
...
from pipelines import batch_scrub_youtube_podcast as pipeline  # noqa: E402
...
def test_processes_every_eligible_file() -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        out_dir = Path(d) / "out"
        in_dir.mkdir()
        (in_dir / "a.m4a").write_text("x")
        (in_dir / "b.opus").write_text("x")
        (in_dir / "c.mp4").write_text("ignore me — video, not audio")
        ...
        with patch.object(pipeline.scrub_youtube_podcast, "run", fake_scrub):
            result = pipeline.run(str(in_dir), str(out_dir), force=True, options={"verbose": False})

        check("2 audio files delegated (mp4 ignored)", len(calls) == 2, str(calls))
...
def test_one_failure_does_not_stop_batch() -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        out_dir = Path(d) / "out"
        in_dir.mkdir()
        (in_dir / "ok.m4a").write_text("x")
        (in_dir / "bad.m4a").write_text("x")
